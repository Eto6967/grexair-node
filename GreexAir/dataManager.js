...
async function getLatestSensorData() {
    ...
    try {
        ...
        if (rows.length > MAX_POINTS) {
            ...
            let thinned = rows.filter((_, i) => i % step === 0);
            if (rows.length > 0 && rows[rows.length-1].id !== thinned[thinned.length-1]?.id) {
                thinned.push(rows[rows.length-1]);
            }
            return thinned;
        }
        return rows;
...
async function getHistoryData(dateStr) {
    try {
        ...
        const res = await pool.query(
            `SELECT timestamp as "Ido", co2_ppm as "CO2_ppm" FROM sensor_data WHERE timestamp >= $1 AND timestamp <= $2 ORDER BY timestamp ASC`,
    ...
    if (rawValues.length > 5 && effectiveWindow > 3) {
        try {
            ...
            if (sgResult && sgResult.length < rawValues.length) {
                smoothValues = [...rawValues.slice(0, padSize), ...sgResult, ...rawValues.slice(rawValues.length - padSize)];
            } else { smoothValues = sgResult; }
        } catch (err) { console.error('❌ Savgol hiba:', err.message); }
    }
