...
function getStatus(val) {
    ...
    if (val < 800)  return { text: 'KIVÁLÓ',     key: 'quality_excellent', cssClass: config.CLASS_GOOD };
    if (val < 1000) return { text: 'JÓ',         key: 'quality_good',      cssClass: 'status-ok' };
    if (val < 1500) return { text: 'FIGYELEM',   key: 'quality_warning',   cssClass: config.CLASS_WARNING };
    return                 { text: 'KRITIKUS',   key: 'quality_critical',  cssClass: config.CLASS_DANGER };
}
...
function processAdvancedData(dataArray, windowSize = 15) {
    ...
    let timeExc = 0, timeGood = 0, timeWarn = 0, timeBad = 0;
    const speedBuckets = { calm:0, slow:0, medium:0, fast:0 };
    ...
    for (let i = 0; i < n; i++) {
        ...
        if      (sVal < 800)  timeExc  += dt;
        else if (sVal < 1000) timeGood += dt;
        else if (sVal < 1500) timeWarn += dt;
        else                  timeBad  += dt;

        // Szellőzési sebesség eloszlás
        const spd = Math.abs(speed[i] || 0);
    ...
    return {
        chartData,
        timeStats: [+timeExc.toFixed(1), +timeGood.toFixed(1), +timeWarn.toFixed(1), +timeBad.toFixed(1)],
        speedBuckets
    };
}
...
async function getFullDayData(dateStr) {
    try {
        ...
        let tExc = 0, tGood = 0, tWarn = 0, tBad = 0;
        ...
        return {
            ...
            timeStats:    [+tExc.toFixed(1), +tGood.toFixed(1), +tWarn.toFixed(1), +tBad.toFixed(1)],
            speedBuckets,
            chartPoints
        };
    } catch (err) {
...
async function getDailyHeatmap() {
    try {
        const res = await pool.query(`
            SELECT
                DATE(timestamp) AS day,
                ROUND(AVG(co2_ppm))::int AS avg_ppm,
                COUNT(*) AS cnt
            FROM sensor_data
        ...
        console.error('getDailyHeatmap hiba:', e.message);
        return [];
    }
}
...
module.exports = {
    ...
    getFullDayData, hendersonFilter, getDailyHeatmap
};
