...
function prepPayload(data, isLive = true) {
    ...
    try {
        ...
        return {
            ...
            speedBuckets:     advanced.speedBuckets || {},
...
app.get('/report', async (req, res) => {
    try {
        ...
        // Compute status server-side so EJS doesn't need dataManager
        let airStatus = { text: '-', key: 'air_nodata', cssClass: 'status-neutral' };
        if (full && full.kpi) {
            airStatus = dataManager.getStatus(full.kpi.current);
        }

        res.render('report', { full, airStatus, date: dateStr, tr, lang, theme });
    } catch(err) {
...
// ── Hőtérkép: napi átlagok ──────────────────────────────────
app.get('/api/heatmap', async (req, res) => {
    try {
        const data = await dataManager.getDailyHeatmap();
        res.json(data);
    } catch(err) {
        console.error('Heatmap hiba:', err.message);
        res.json([]);
    }
});
